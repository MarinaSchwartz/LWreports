...
---
title: "lakewatch lake template batch processing"
output:
  pdf_document: default
  html_document: default
params: 
...
```{r data_processing,  eval=TRUE, include=FALSE}
...
########################################################################################
  ###commenting this code out so i can test the for loop.  


data_1 = read_csv("All_Data.csv")

data_1 = data_1 %>%
  mutate(Lake_County = paste(Lake, County))

#data_2 = read_xlsx("Lakewatch Base File 10-24-2023.xlsx")


#data_2 <- data_2 %>%
 # mutate(Lake_County = paste(Lake, County))

lake = data_1 %>% filter(Lake_County == "Alice Alachua") #subsetting data for lake alice
...
### merge test
#data_lw = data_1 %>% filter(Study == "LW") %>% filter(`water type` != "Lake")
#merging data will be tricky because the data_1 is longer, not sure off hand how to merge the data into a master set. 
...
```{r table_one_image, include=TRUE}
...
data_means = lake%>% group_by(Lake_County) %>% summarise(
...
data_mins = lake %>% group_by(Lake_County) %>%  summarise(
...
data_max = lake %>% group_by(Lake_County) %>% summarise(
...
gtsave(table_two, filename = "table_2.png", path = "output/output_table")
...
```{r}
knitr::include_graphics("output/output_table/table_2.png")
```
...
plot_title = glue("Total Phosphorus (µg/L) by Year for Lake {lake$Lake[1]} in {lake$County[1]} County")  
  

label = (glue("p = {signif(total_p_table$p.value, digits = 2)}, R<sup>2</sup> = {signif(total_p_table$r.squared, digits = 2)}, {trend} "))

maxlim = max(lake$TP)+10
minlim = min(lake$TP)
...
total_p_graph = ggplot(data = lake, aes(x = Year, y = TP)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) + geom_richtext(
    label = label
    ,x = (min(lake$Year, na.rm = TRUE)+5),
    y = (max(lake$TP, na.rm = TRUE)+5),
    )+ylim(minlim, maxlim)
...
```{r n_graph}
total_n_lm = lm(TN ~ Year, data = lake)

total_n_table = glance(total_n_lm)

trend = if_else(total_n_table$p.value >= 0.5, true = "No trend", false = if_else(total_n_lm[["coefficients"]][["Year"]] > 0 , true = "Increasing", false = "Decreasing"))
  
plot_title = glue("Total Nitrogen (µg/L) by Year for Lake {lake$Lake[1]} in {lake$County[1]} County")  
  

label = (glue("p = {signif(total_n_table$p.value, digits = 2)}, R<sup>2</sup> = {signif(total_n_table$r.squared, digits = 2)}, {trend} "))

maxlim = max(lake$TN)+10
minlim = min(lake$TN)

total_n_graph = ggplot(data = lake, aes(x = Year, y = TN)) +
  geom_point() +
  geom_smooth(
    method = "lm", 
    se = FALSE, 
...
```{r chl_graph}
total_chl_lm = lm(CHL ~ Year, data = lake)

total_chl_table = glance(total_chl_lm)

trend = if_else(total_chl_table$p.value >= 0.5, true = "No trend", false = if_else(total_chl_lm[["coefficients"]][["Year"]] > 0 , true = "Increasing", false = "Decreasing"))
  
plot_title = glue("Total Chlorophyll (µg/L) by Year for Lake {lake$Lake[1]} in {lake$County[1]} County")  
  

label = (glue("p = {signif(total_chl_table$p.value, digits = 2)}, R<sup>2</sup> = {signif(total_chl_table$r.squared, digits = 2)}, {trend} "))

maxlim = max(lake$CHL)+10
minlim = min(lake$CHL)

total_chl_graph = ggplot(data = lake, aes(x = Year, y = CHL)) +
  geom_point() +
  geom_smooth(
    method = "lm", 
    se = FALSE, 
    linetype = paste(
      if_else(total_chl_table$p.value <= 0.5,true = "solid", false = "dashed" )
      )
    ) +
...
```{r secchi_graph}
total_secchi_lm = lm(SECCHI_ft ~ Year, data = lake)
...
plot_title = glue("Secchi Depth (ft) by Year for Lake {lake$Lake[1]} in {lake$County[1]} County")  
  

label = (glue("p = {signif(total_secchi_table$p.value, digits = 2)}, R<sup>2</sup> = {signif(total_secchi_table$r.squared, digits = 2)}, {trend} "))

maxlim = max(lake$SECCHI_ft, na.rm = TRUE)+10
minlim = min(lake$SECCHI_ft, na.rm = TRUE)

total_secchi_graph = ggplot(data = lake, aes(x = Year, y = SECCHI_ft)) +
  geom_point() +
  geom_smooth(
    method = "lm", 
    se = FALSE, 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + geom_richtext(
    label = label,
    x = (min(lake$Year, na.rm = TRUE)+5),
    y = (max(lake$SECCHI_ft, na.rm = TRUE)+5)
    )+ylim(minlim, maxlim)
