...
lake_2 = lake_2 %>% mutate(trophic_state = ifelse(
  gmean(CHL) < 3, "Oligotrophic", ifelse(
    gmean(CHL) >= 3 & gmean(CHL) < 7, "Mesotrophic", ifelse(
      gmean(CHL) >= 7 & gmean(CHL) < 40, "Eutrophic", "Hypertrophic"))))
...
data_means = lake_1 %>% group_by(Lake) %>% summarise(
  "Total Phosphorus (µg/L)" = gmean(TP, na.rm = TRUE),
  "Total Nitrogen (µg/L)" = gmean(TN, na.rm = TRUE),
  "Chlorophyll- uncorrected (µg/L)" = gmean(CHL, na.rm = TRUE),
  "Secchi (ft)" = gmean(SECCHI_ft, na.rm = TRUE),
  "Secchi (m)" = gmean(SECCHI_ft, na.rm = TRUE)*0.3,
  "Color (Pt-Co Units)" = gmean(Color, na.rm = TRUE),
  "Specific Conductance (µS/cm@25 C)" = gmean(Cond_uS, na.rm = TRUE),
  "Lake Classification" = 
