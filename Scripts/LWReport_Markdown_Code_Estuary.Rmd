...
#adding a column for Estuary
 #Lake_2 = Lake_2 %>% mutate(Estuary = Lake_1$Estuary)
  Lake_2$Estuary = paste0(Lake_1$Estuary)
    
#adding a column for Estuary Segment
 #Lake_2 = Lake_2 %>% mutate(Estuary_Segment = Lake_1$Estuary_Segment)
 Lake_2$Estuary_Segment = paste0(Lake_1$Estuary_Segment)

#adding a column for NNCwaters
 #Lake_2 = Lake_2 %>% mutate(Narrative_Nutrient_Criteria_Waters = Lake_1$Narrative_Nutrient_Criteria_Waters)
 Lake_2$Narrative_Nutrient_Criteria_Waters = paste0(Lake_1$Narrative_Nutrient_Criteria_Waters)
```

```{r title,  echo=FALSE}
report_title = glue("Florida LAKEWATCH Report for  {Lake_2$Lake[1]} in {Lake_2$County[1]} — {format(Sys.Date(), '%B %d, %Y')}")
estuary = glue("{ifelse(is.na(Lake_2$Estuary[1]), '', Lake_2$Estuary[1])}")
estuary_segment = glue("{ifelse(is.na(Lake_2$Estuary_Segment[1]), '', Lake_2$Estuary_Segment[1])}")
NNCwaters = glue("{ifelse(is.na(Lake_2$Narrative_Nutrient_Criteria_Waters[1]), '', Lake_2$Narrative_Nutrient_Criteria_Waters[1])}")
```

## `r report_title`

## Estuary and Estuary Segment: `r estuary` `r estuary_segment` `r NNCwaters`
...
The first part of this summary lists background data for each system, the second part lists the long-term data averages and ranges and the final part are trend plots for nutrients, chlorophyll, and Secchi depth.
Plots were only made for systems with five or more years of data.
...
#make gt table
table_one = gt(tbl2df) %>% tab_options(column_labels.hidden = TRUE) %>% fmt_number(
  decimals = 1
) %>% opt_table_font(
  stack = "transitional"
...
gtsave(table_one, filename = "table_1.png", path = "~/Documents/GitHub/LWreports/Output/Estuary/output_table")

```

```{r table 1, out.width= "500px", fig.align='center'}
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Estuary/output_table/table_1.png")
```
...
**Figure 1 and Figure 2. Trend plots of total phosphorus and total nitrogen versus year. The R^2^ value indicates the strength of the relations (ranges from 0.0 to 1.0; higher the R2 the stronger the relation) and the p value indicates if the relation is significant (p \< 0.05 is significant). Trend Status are reported on plots as Increasing, Decreasing, or No Trend.**
...
ggsave("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_p_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
```

```{r total p_graph out, out.width= "400px",fig.align = "center"}
if(tp_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_p_graph.png")
}
```
...
ggsave("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_n_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
```

```{r total n_graph out, out.width= "400px",fig.align = "center"}
if(tn_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_n_graph.png")
}
```

\newpage

**Figure 3 and Figure 4. Trend plots of total chlorophyll and Secchi depth versus year. The R^2^ value indicates the strength of the relations (ranges from 0.0 to 1.0; higher the R2 the stronger the relation) and the p value indicates if the relation is significant (p \< 0.05 is significant). Trend Status are reported on plots as Increasing, Decreasing, or No Trend.**
...
ggsave("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_chl_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
```

```{r chl_graph out, out.width= "400px",fig.align = "center"}
if(chl_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs//total_chl_graph.png")
}
```
...
trend = if_else(
  total_secchi_table$p.value >= 0.05,
  true = "No trend",
  false = if_else(
...
ggsave("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_secchi_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
...
if(secchi_m_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Estuary/output_graphs/total_secchi_graph.png")
}
```
