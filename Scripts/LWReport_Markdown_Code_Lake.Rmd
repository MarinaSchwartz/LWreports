...
#This is in the for loop code - redundant here 
...
#     gmean(`Color`) <= 40 & gmean(`Cond_uS`) <= 100, "Clear Soft Water","Clear Hard Water")))
# 
...
report_title <- glue::glue(
  "Florida LAKEWATCH Report for {Lake_2$Lake[1]} in {Lake_2$County[1]} — {format(Sys.Date(), '%B %d, %Y')}"
)
```
...
```{r table_one_image, echo=FALSE, out.width= "450px", fig.align='center',}
knitr::include_graphics("~/Documents/GitHub/LWreports/Images/LW Reports Table 1 V.2.png")
...
max(byyr$`gmean(Color)`[ is.na(byyr$`gmean(Color)`) == FALSE]), 
max(byyr$`gmean(Cond)`[is.na(byyr$`gmean(Cond)`) == FALSE]),
" ")
...
gtsave(table_two, filename = "table_2.png", path = "~/Documents/GitHub/LWreports/Output/Lake/output_table")

```

```{r table 2, out.width= "500px", fig.align='center'}
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Lake/output_table/table_2.png")
```
...
#mutate cond_Us to numeric
Lake_2 <- Lake_2 %>%
  mutate(Cond_uS = as.numeric(Cond_uS))
...
lts <- Lake_2$trophic_state[1] #Trophic State
tpz <- Lake_1$`TP Zone`[1] #tp zone
gtp <- paste(ggtp, "(", min(byyr$`gmean(TP)`[is.na(byyr$`gmean(TP)`)== FALSE]), " to ", max(byyr$`gmean(TP)`[is.na(byyr$`gmean(TP)`)== FALSE]) , ")" ) #grand total phosphorous
tnz <- Lake_1$`TN Zone`[1]
gtn <- paste(ggtn, "(", min(byyr$`gmean(TN)`[is.na(byyr$`gmean(TN)`)== FALSE]), " to ", max(byyr$`gmean(TN)`[is.na(byyr$`gmean(TN)`)== FALSE]) , ")" )  #grand tn geomean
...
#add row names to array
rowname <- c("County", "Name", "GNIS Number", "Latitude", "Longitude", "Water Body Type", "Surface Area (ha and acre)", "Period of Record (year)", "Lake Trophic Status (CHL)", "TP Zone", "Grand TP Geometric Mean Concentration (ug/ L, min, and max.)", "TN Zone", "Grand TN Geometric Mean Concentration (ug/ L, min, and max.)")
...
gtsave(table_three, filename = "table_3.png", path = "~/Documents/GitHub/LWreports/Output/Lake/output_table/")

```

```{r table 3, out.width= "500px", fig.align='center'}
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Lake/output_table/table_3.png")
```

```{r Figure_one_image, echo=FALSE, out.width= "6.5in", fig.align='center',}

knitr::include_graphics("~/Documents/GitHub/LWreports/Images/LW Reports Figure 1.png")
...
ggsave("~/Documents/GitHub/LWreports/Output/Lake/output_graphs/total_n_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
```

```{r total n_graph out, out.width= "400px",fig.align = "center"}
if(tn_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Lake/output_graphs/total_n_graph.png")
}
```
\newpage

**Figure 4 and Figure 5. Trend plots of total chlorophyll and Secchi depth versus year. The R^2^ value indicates the strength of the relations (ranges from 0.0 to 1.0; higher the R2 the stronger the relation) and the p value indicates if the relation is significant (p \< 0.05 is significant). Trend Status are reported on plots as Increasing, Decreasing, or No Trend.**
...
total_secchi_graph = ggplot(data = Lake_2_bymo_SECCHI_ft, aes(x = Year, y = gmean.SECCHI_ft)) +
  ...
  geom_smooth(
    ...
    alpha=0.15,
    linetype = 
    paste( if_else(total_secchi_table$p.value <= 0.05,true = "solid", false = "dashed" ) ) ) +
  ###If stats are NaN then change line type to "dashed" and remove if else statement
  ##"dashed")+
...
ggsave("~/Documents/GitHub/LWreports/Output/Lake/output_graphs/total_secchi_graph.png", width = 6, height = 4, units = "in", dpi = 300)
}
...
if(secchi_m_n$n >= 5){
knitr::include_graphics("~/Documents/GitHub/LWreports/Output/Lake/output_graphs/total_secchi_graph.png")
}
```
