...
#lm for graph to refer to 
  ...
  mutate(TP = as.numeric(TP), na.rm=TRUE) %>%
  summarise(TP = gmean(TP), na.rm=TRUE)
  
total_p_lm = lm(TP ~ Year, data = Lake_2_bymo_TP)
...
total_p_graph <- ggplot(data = Lake_2_bymo_TP, aes(x = Year, y = TP)) +
