...
This report summarizes data collected on systems that have been part of the LAKEWATCH program. Data are from the period of record for individual systems. The first part of this summary lists background data for each system, the second part lists the long-term data averages and ranges and the final part are trend plots for nutrients, chlorophyll, and Secchi depth. Plots were only made for systems with five or more years of data.

The near shore Florida coastline is separated into estuary and estuary segments within the estuary. Deeper coastal waters are separated into coastal nutrient regions and coastal nutrient segments within the regions. Numeric nutrient criteria are established for all estuary segments, including criteria for total nitrogen, total phosphorus, and chlorophyll a.  For open ocean coastal waters, numeric criteria are established for chlorophyll a, that is derived from satellite remote sensing techniques. For those locations without defined segments there are narrative nutrient criteria (e.g., Florida Keys Halo Zone).
...
The individual nutrient criteria can be found at the following link:
https://www.flrules.org/gateway/RuleNo.asp?title=SURFACE%20WATER%20QUALITY%20STANDARDS&ID=62-302.532
...
table_one = gt(tbl1df) %>% tab_options(column_labels.hidden = TRUE) %>% fmt_number(
  ...
  title = md("**Table 1.** Base File Data."))  

gtsave(table_one, filename = "table_1.png", path = "output/output_table")

```

```{r table 1, out.width= "500px", fig.align='center'}
knitr::include_graphics("output/output_table/table_1.png")
```
...
### Table 2. Long-term trophic state data collected monthly by LAKEWATCH volunteers and color and specific conductance (collected quarterly).
...
Figure 1 and Figure 2. Trend plots of annual average total phosphorus and annual average total nitrogen versus year. The R2 value indicates the strength of the relations (ranges from 0.0 to 1.0; higher the R2 the stronger the relation) and the p value indicates if the relation is significant (p \< 0.05 is significant). Trend Status are reported on plots.
...
ggsave("output/output_graphs/total_p_graph.png", width = 8, height = 6, units = "in", dpi = 300)

```

```{r total p graph out, out.width= "450px",fig.align = "center"}

knitr::include_graphics("output/output_graphs/total_p_graph.png")
```
...
ggsave("output/output_graphs/total_n_graph.png", width = 8, height = 6, units = "in", dpi = 300)
...
knitr::include_graphics("output/output_graphs/total_n_graph.png")

```

Figure 3 and Figure 4. Trend plots of annual average chlorophyll and annual average Secchi versus year. The R2 value indicates the strength of the relations (ranges from 0.0 to 1.0; higher the R2 the stronger the relations and the p value indicates if the relation is significant (p \< 0.05 is significant). Trend status are reported on plots.
...
##commented out so the graph does not show up twice.
ggsave("output/output_graphs/total_chl_graph.png", width = 8, height = 6, units = "in", dpi = 300)

```

```{r chl graph out, out.width= "500px",fig.align = "center"}

knitr::include_graphics("output/output_graphs/total_chl_graph.png")
...
###commented out so the graph does not show up twice.
ggsave("output/output_graphs/total_secchi_graph.png", width = 8, height = 6, units = "in", dpi = 300)

```

```{r secchi_graph out, out.width= "500px",fig.align = "center"}

knitr::include_graphics("output/output_graphs/total_secchi_graph.png")
